...
namespace Components.Enemies.States
{
    public class RotatingToContinue : AEnemiesState
    {
        ...
        {
            currentTransform.rotation = Quaternion.RotateTowards(currentTransform.rotation, rightRotation, rotateSpeed * Time.fixedDeltaTime);

            float angle = Quaternion.Angle(currentTransform.rotation, rightRotation);
            if(angle < rotateSpeed*Time.fixedDeltaTime)
            {
                currentTransform.rotation = rightRotation;
